...
extern void UartSendString(UINT8 *txbuf,UINT32 buflen); 
extern 
#define __USBD_FULL_SPEED_MODE__ 1 
...
{  
	...
	usbdInfo.u32HIDRPTDescriptorLen = USB_DT_HID_RPT;
	...
	outp32(USB_IRQ_ENB, (DMACOM_IE | RUM_IE | RST_IE|VBUS_IE|SOF_IE));
	outp32(ADDR, 0);
#ifndef __USBD_FULL_SPEED_MODE__		
...
VOID usbd_update_device(VOID)
{
	...
	switch(_usb_cmd_pkt.bRequest)
	{
		...
		case USBR_CLEAR_FEATURE:
			...
			if(usbdInfo.disableremotewakeup == 1)
			{
		...
		|| (usbdInfo.GET_HID_Flag == 1) || (usbdInfo.GET_HID_RPT_Flag == 1)) 
	{
		if (usbdInfo._usbd_remlen_flag == 0)
		{
			if (usbdInfo.GET_DEV_Flag == 1)
			{
				if(getMaxLun == 1)
				{
					ptr = (UINT32 *)usbdInfo.pu32StringDescriptor[1];
					getMaxLun=0;
				}
				else
...
VOID usbd_control_packet(VOID)
{
	...
	//zhangys 2013-7-26 12:12:17 解决部分电脑无法正常启动
	if ((_usb_cmd_pkt.bmRequestType &0xE0) == 0xa0)     /* 0xA1 or 0xA2 is Class Get Request */
	{
			_usb_cmd_pkt.bRequest = USBR_GET_DESCRIPTOR;
			getMaxLun = 1;     
	}         

	if (_usb_cmd_pkt.bmRequestType == 0x21)//if ((_usb_cmd_pkt.bmRequestType &0xE0) == 0x20)     //0x21 or 0x22 is Class Set Request
    {
	...
	if (_usb_cmd_pkt.bmRequestType == 0x21)
	{
	...
	switch (_usb_cmd_pkt.bRequest)
	{
		case USBR_GET_DESCRIPTOR:
			//zhangys 2013-7-26 12:12:17 ?a??2?·?μ????T·¨?y3￡???ˉ
			if(getMaxLun == 0)
			{
				if(_usb_cmd_pkt.bmRequestType==0x80||_usb_cmd_pkt.bmRequestType==0x81)		
					ReqErr =0;
				else
					ReqErr =1;
...
VOID usbd_isr(VOID)
{
	...
	if (IrqStL & IRQ_USB_STAT)
	{
		...
		if (IrqSt & SOF_IS & IrqEn)
		{
			...
			//zhangys 2014年7月9日11:07:21 防止静电后无法重启枚举
			gudSofCnt++;
		...
		if (IrqSt & RUM_IS & IrqEn)
		{
			...
			outp32(USB_IRQ_ENB, (USB_RST_STS|USB_SUS_REQ|VBUS_IE|SOF_IE));
		}
...
#ifdef __USBD_FULL_SPEED_MODE__				
			outp32(USB_IRQ_ENB, (USB_RST_STS|USB_RESUME|VBUS_IE|SOF_IE));
#endif			
	...
	if (IrqStL & IRQ_CEP) 
	{
		...
		{		
		    ...
		    {
				outp32(CEP_IRQ_STAT, (CEP_STACOM_IS|CEP_DATA_RxED_IS));
				outp32(CEP_CTRL_STAT, CEP_NAK_CLEAR);		/* clear nak so that sts stage is complete */
				outp32(CEP_IRQ_ENB,  (CEP_STACOM_IE|CEP_PING_IE|CEP_DATA_TxED_IE|CEP_SETUP_PK_IE|CEP_OUT_TK_IE|CEP_IN_TK_IE));		// suppkt int//enb sts completion int
