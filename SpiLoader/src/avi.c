...
{
	UINT16 u16HearPhone;		
	gpio_setportval(GPIO_PORTB, 0x04, 0x00);			//GPIOB2 input mode 
	gpio_readport(GPIO_PORTB, &u16HearPhone);
#if defined(__IXL_WINTEK__)	
	if( (u16HearPhone & 0x04) ==0x00 )					
#elif defined(__GWMT9360A__) || defined(__GWMT9615A__)
	outp32(REG_GPEFUN, inp32(REG_GPEFUN) & (~(MF_GPE0 | MF_GPE1)));
	gpio_setportval(GPIO_PORTE, 0x01, 0x00);			//GPIOE0 input mode 
...
void startAmpEnableCheck(void)
{
	...
	tmp = sysSetTimerEvent(TIMER0, 10, (PVOID)ampEnable);	/* 10 ticks = 100ms call back */
...
{
#if defined(__LCM_800x600__) || \
	defined(__LCM_800x480__) 
...
#endif	
#if defined(__IXL_WINTEK__) || \
	defined(__LCM_480x272__)
	outp32(REG_GPDFUN, inp32(REG_GPDFUN) & (~MF_GPD1));		//!!!! Remember switch GPIOD 1 to GPIO. It will cause the ICE crash. 
	gpio_setportval(GPIO_PORTD, 0x02, 0x02);			//GPIOD1 output high 
	gpio_setportpull(GPIO_PORTD, 0x02, 0x02);			//GPIOD1 pull high
	gpio_setportdir(GPIO_PORTD, 0x02, 0x02);			//GPIOD1 output mode
#endif
}
